...
describe("fragmentation", () => {
  test("single author entity has fractal value 0.0", () => {
    ...
    expect(result).toEqual([
      { entity: "A", "fractal-value": "0.0", "total-revs": 2 },
    ]);
  });
  ...
  test("calculates fractal value for multiple authors", () => {
    ...
    // fractal = 1 - ((9/36) + (4/36) + (1/36)) = 1 - 14/36 = 0.61 (2 sig figs)
    const result = fragmentation(data, lowThresholds);
    expect(result).toEqual([
  ...
  test("sorts by fractal-value descending", () => {
    const data: Modification[] = [
      // Entity B: single author -> fractal = 0.0
    ...
    expect(result).toEqual([
      ...
      { entity: "B", "fractal-value": "0.0", "total-revs": 1 },
    ]);
  });
